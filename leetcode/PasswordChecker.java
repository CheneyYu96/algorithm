...
public class Solution {
	...
	// 可以通过删除，减少修改的次数。尽量选择效果明显的删除：删除一个可以抵一次替换>删除2个抵一次替换>删除三个低一次替换
            if(del<=delete[0]) change-=del;
            else if(del-delete[0]<=2*delete[1]) change = change - (delete[0]+(del-delete[0])/2);
            else change = change - (delete[0]+delete[1]+(del-delete[0]-2*delete[1])/3);
